...
.post(cors.corsWithOptions, authenticate.verifyUser,(req,res,next) => {
    Favorites.findOne({ user: req.user._id }, (err, favorite) => {
        ...
        if (favorite != null){
            err = new Error('The favortie of this dish is already Added!');
            err.status = 404;
            return next(err);
        }
        else {
    ...
    Favorites.findOne({ user: req.user._id }, (err, favorite) => {
        ...
        if (!favorite) {
            ...
            if (favorite.dishes.indexOf(req.params.dishId) < 0 ) {
                ...
                res.statusCode = 403;
                res.setHeader('Content-Type', 'text/plain');
                res.end('Dish '+ req.params.dishId + ' already exists in your Favorite');
            }
        }
    });
})
    ...
    Favorites.findOne({ user: req.user._id }, (err, favorite) => {
        ...
        if (index >= 0 ) {
            ...
            .then((favorite) => {
                ...
                .populate('dishes')
                .then((favorite) => {
                    console.log('Your Favorite Dish Deleted!', favorite);
                    res.statusCode = 200;
                    res.setHeader('Content-Type', 'application/json');
                    res.json(favorite);
                })
